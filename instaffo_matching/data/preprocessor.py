...
def standardize_data(
    talent_df: pd.DataFrame, job_df: pd.DataFrame
    ...
    job_df["seniorities"] = job_df["seniorities"].apply(
        lambda x: [seniority_hierarchy[sen] for sen in x]
    )

    # Encoding languages
    def encode_languages(lang_list):
        return [
            {**lang, "rating": language_hierarchy.get(lang["rating"], 0)} for lang in lang_list
        ]
