...
class FeatureEngineer:
    ...
    def fit(self, job_df: pd.DataFrame, talent_df: pd.DataFrame):
        ...
        all_job_roles = (
            job_df["job_roles"].apply(" ".join) + " " + talent_df["job_roles"].apply(" ".join)
        )
    ...
    def engineer_features(self, job_df: pd.DataFrame, talent_df: pd.DataFrame):
        ...
        for idx in job_df.index:
            ...
            language_match_score = self._calculate_language_match(
                talent["languages"], job["languages"]
            )
            ...
            salary_difference_percentage = (
                talent["salary_expectation"] - job["max_salary"]
            ) / job["max_salary"]
    ...
    def _calculate_language_match(
        self, talent_languages: List[Dict], job_languages: List[Dict]
    ) -> float:
        """
        Calculates the language match score between talent and job.

        Since must-have languages are
